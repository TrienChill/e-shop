...
function isRefreshTokenError(error: unknown): boolean {
  ...
  const isMessageMatch = REFRESH_TOKEN_PATTERNS.some(
...
function wrapAuthClient(client: SupabaseClient): SupabaseClient {
  ...
  client.auth.signInWithOAuth = async (...args: Parameters<typeof originalSignInWithOAuth>) => {
    try {
      ...
      if (isRefreshTokenError(error)) {
        ...
        return { data: { url: null, provider, session: null }, error: error as AuthError };
      }
      throw error;
    }
  };

  client.auth.signInWithOtp = async (...args: Parameters<typeof originalSignInWithOtp>) => {
    try {
