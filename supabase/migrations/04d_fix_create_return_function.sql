-- Fix: Thêm default cho tất cả params sau p_request_type
-- Chạy trong Supabase SQL Editor

DROP FUNCTION IF EXISTS public.create_return_request(
  bigint, uuid, varchar, return_reason_type, text, text, text[], varchar, text, text, text, text
);

DROP FUNCTION IF EXISTS public.create_return_request(
  bigint, uuid, varchar, return_reason_type, text, text, text[], varchar, text, text, text, jsonb
);

CREATE OR REPLACE FUNCTION public.create_return_request(
  p_order_id        bigint,
  p_user_id         uuid,
  p_request_type    varchar(20) DEFAULT 'return',
  p_reason_category return_reason_type DEFAULT 'other',
...
BEGIN
  v_items_data := COALESCE(p_items, '[]'::jsonb);
