...
export default function AdminVouchersScreen() {
  ...
  const handleSaveDiscount = async (data: Partial<ProductDiscountRow>) => {
    try {
      if (editingDiscount) {
  ...
  const handleDelete = (id: string, codeOrName: string | null, type: "voucher" | "discount") => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        ...
        >
          <Text style={[styles.tabText, activeTab === "VOUCHERS" && styles.tabTextActive]}>Mã giảm giá (Sàn)</Text>
        </Pressable>
        <Pressable
          style={[styles.tabBtn, activeTab === "PRODUCT_DISCOUNTS" && styles.tabBtnActive]}
          onPress={() => setActiveTab("PRODUCT_DISCOUNTS")}
        >
          <Text style={[styles.tabText, activeTab === "PRODUCT_DISCOUNTS" && styles.tabTextActive]}>Giảm giá Sản phẩm</Text>
        </Pressable>
      </View>
      ...
      <View style={styles.tableCard}>
        ...
        {activeTab === "VOUCHERS" ? (
          ...
          <View style={styles.tableHeader}>
            <Text style={StyleSheet.flatten([styles.columnCode, styles.headerText])}>ID Sản phẩm</Text>
            <Text style={StyleSheet.flatten([styles.columnValue, styles.headerText, styles.textRight])}>Giá trị giảm</Text>
            <Text style={StyleSheet.flatten([styles.columnExpired, styles.headerText, styles.textCenter])}>Thời hạn</Text>
            <Text style={StyleSheet.flatten([styles.columnStatus, styles.headerText, styles.textCenter])}>Trạng thái</Text>
            <Text style={StyleSheet.flatten([styles.columnActions, styles.headerText, styles.textRight])}>Hành động</Text>
          </View>
        )}
                    ...
                    <Pressable
                      ...
                      onPress={() => {
                        if (status.color !== "expired") {
                          setVoucherActive(v.id, !v.is_active).then(() => refresh()).catch((e) => setError((e as Error).message));
                        }
                      }}
                    >
      ...
      <AdminProductDiscountModal
        ...
        onClose={() => { setDiscountModalVisible(false); setEditingDiscount(null); }}
...
const styles = StyleSheet.create({
  ...
  tabsContainer: { flexDirection: "row", backgroundColor: "#F3F4F6", padding: 4, borderRadius: 12, marginBottom: 8 },
