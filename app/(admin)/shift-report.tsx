...
export default function ShiftReportScreen() {
  ...
  const cancelledOrders = orders.filter(
    (o) => o.status === "cancelled" || o.status === "delivery_failed",
  ).length;
...
Viết bằng tiếng Việt, ngắn gọn, súc tích, văn phong chuyên nghiệp nhưng gần gũi. Không dùng markdown code block, chỉ dùng **in đậm** cho tiêu đề và dấu - cho danh sách.`;
      ...
      setAiInsight(
        `Đã xảy ra lỗi khi tạo báo cáo: ${err.message ?? "Lỗi không xác định"}`,
      );
    } finally {
      setAiLoading(false);
    }
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <Pressable
            style={[styles.btnExport, { backgroundColor: "#8B5CF6" }]}
      ...
      <View style={styles.tableCard}>
        <Text style={styles.tableTitle}>
          Danh sách đơn hàng ngày {format(selectedDate, "dd/MM/yyyy")}
        </Text>
        ...
        {loading ? (
          ...
          orders.map((o) => (
            <AdminDataWrapper
              key={o.id}
              onPress={() => router.push(`/(admin)/orders/${o.id}` as any)}
              style={styles.row}
            >
              <Text
                style={[
                  styles.td,
                  { flex: 1, fontWeight: "600", color: "#374151" },
                ]}
              >
                {encodeOrderId(o.id)}
              </Text>
              <Text style={[styles.td, { flex: 2 }]}>
                {o.receiver_name || "Khách"}
              </Text>
              ...
              {aiLoading ? (
                <View style={styles.aiLoadingContainer}>
                  ...
                  <Text style={styles.aiLoadingText}>
                    AI đang phân tích ca làm việc ngày{" "}
                    {format(selectedDate, "dd/MM/yyyy")}...
                  </Text>
                </View>
              ) : (
            ...
            {!aiLoading && (
              <View style={styles.aiModalFooter}>
                <Text style={{ fontSize: 12, color: "#9CA3AF" }}>
                  Báo cáo được tạo bởi Gemini AI
                </Text>
              </View>
            )}
          </View>
