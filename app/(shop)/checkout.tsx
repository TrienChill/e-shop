...
const GuestCustomerForm = React.memo(
  ({
    ...
    const sectionStyle =
      isWeb && ws ? ws.guestFormSection : styles.guestFormSection;
...
export default function CheckoutScreen() {
  ...
  const reloadAddresses = async () => {
    try {
      ...
      const { data: addresses } = await supabase
        .from("user_addresses")
        .select(
          "id, receiver_name, phone_number, province_city, district, ward_commune, street_address, ghn_district_id, ghn_ward_code, is_default",
        )
        .eq("user_id", user.id)
      ...
      if (addresses && addresses.length > 0) {
        ...
        const currentStillExists = addresses.find(
          (a) => a.id === userAddress?.id,
        );
        if (!currentStillExists) {
          handleSelectAddress(addresses[0]);
        } else {
          const updatedCurrent = addresses.find(
            (a) => a.id === userAddress?.id,
          );
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      const finalPayload = editAddressData.id
        ? { ...payload, id: editAddressData.id }
        : payload;
  ...
  const handleSelectAddress = (addr: any) => {
    ...
    if (addr.ghn_district_id)
      setCustomerDistrictId(Number(addr.ghn_district_id));
      ...
      try {
        ...
        if (!isAnon) {
          ...
          if (addresses && addresses.length > 0) {
            ...
            if (defaultAddr.ghn_district_id)
              setCustomerDistrictId(Number(defaultAddr.ghn_district_id));
            if (defaultAddr.ghn_ward_code)
              setCustomerWardCode(String(defaultAddr.ghn_ward_code));
          } else {
            setAllAddresses([]);
            setCustomerName("");
            setCustomerPhone("");
  ...
  const handlePlaceOrder = async () => {
    ...
    if (isGuest) {
      if (
        ...
        Alert.alert(
          "Thiếu thông tin",
          "Vui lòng điền đầy đủ Tên, Số điện thoại và Địa chỉ giao hàng.",
        );
        return;
      }
    ...
    try {
      ...
      if (isGuest) {
        ...
        orderPayload = {
          ...
          shipping_district_id: guestDistrictId
            ? Number(guestDistrictId)
            : null,
        ...
        orderPayload = {
          ...
          shipping_method_id: selectedShippingId,
          shipping_district_id: userAddress.ghn_district_id
            ? Number(userAddress.ghn_district_id)
            : null,
          shipping_ward_code: userAddress.ghn_ward_code
            ? String(userAddress.ghn_ward_code)
            : null,
        };
      }
  ...
  return (
    <SafeAreaView
      style={IS_WEB_DESKTOP ? webStyles.safeArea : styles.container}
    >
              ...
              {isGuest ? (
                ...
                <View style={webStyles.addressCard}>
                  <View style={webStyles.addressCardHeader}>
                    <Text style={webStyles.addressCardTitle}>
                      Địa chỉ giao hàng
                    </Text>
                  ...
                  {loading ? (
                    ...
                    <View>
                      <Text style={webStyles.receiverName}>
                        {userAddress.receiver_name} | {userAddress.phone_number}
                      </Text>
                      <Text style={webStyles.addressText}>
                        {userAddress.street_address},{" "}
                        {userAddress.ward_commune
                          ? userAddress.ward_commune + ", "
                          : ""}
                        {userAddress.district}, {userAddress.province_city}
                      </Text>
                    </View>
                  ) : (
                    <Text style={webStyles.noAddress}>
            ...
            <View style={webStyles.rightCol}>
              <View style={webStyles.stickySummary}>
                ...
                {loading ? (
                    ...
                    <View key={item.id} style={webStyles.summaryItem}>
                      <View style={webStyles.summaryItemImgWrap}>
                        <Image
                          ...
                          style={webStyles.summaryItemImg}
                        />
                        <View style={webStyles.summaryItemQtyBadge}>
                          <Text style={webStyles.summaryItemQty}>
                            {item.quantity}
                          </Text>
                        </View>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text
                          style={webStyles.summaryItemName}
                          numberOfLines={2}
                        >
                          {item.name}
                        </Text>
                        <Text style={webStyles.summaryItemMeta}>
                  ...
                  <Text
                    style={{ color: C.sub, textAlign: "center", padding: 20 }}
                  >
                    Không có sản phẩm
                  </Text>
                )}
                ...
                <View style={webStyles.summaryRow}>
                  ...
                  <Text style={webStyles.summaryValue}>
                ...
                {selectedVoucher && (
                  <View style={webStyles.summaryRow}>
                    ...
                    <Text
                      style={[webStyles.summaryValue, { color: "#EF4444" }]}
                    >
                ...
                {userMembership && userMembership.benefit_percentage > 0 && (
                  <View style={webStyles.summaryRow}>
                    <Text style={webStyles.summaryLabel}>
                      Giảm thành viên ({userMembership.level_name})
                    </Text>
                    <Text
                      style={[webStyles.summaryValue, { color: "#22C55E" }]}
                    >
                ...
                <View style={webStyles.summaryRow}>
                  ...
                  <Text style={webStyles.totalValue}>
            ...
            {isGuest ? (
              ...
              <View style={styles.addressBlock}>
                <View style={styles.addressBlockHeader}>
                  <Text style={styles.addressBlockTitle}>
                    Địa chỉ giao hàng
                  </Text>
                  <TouchableOpacity
                ...
                {loading ? (
                  ...
                  <Text style={styles.noAddressText}>
                    Hiện không có thông tin địa chỉ giao hàng.
                  </Text>
                )}
              </View>
            )}
                  ...
                  >
                    ...
                    <View style={{ flex: 1, paddingRight: 8 }}>
                      ...
                      <Text style={styles.voucherSelectSubtitle}>
                        {selectedVoucher
                          ? selectedVoucher.type === "percentage"
                            ? `Đã áp dụng giảm ${selectedVoucher.discount}%`
                            : `Đã áp dụng giảm ${selectedVoucher.discount.toLocaleString("vi-VN")}₫`
                          : "Chọn hoặc nhập mã"}
                      </Text>
                    </View>
                  </View>
            ...
            {paymentStatus === "success" && (
              <>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setPaymentStatus("idle");
                    // Sửa "/(shop)/(tabs)/home" thành "/" và ép kiểu as any
                    router.push((isGuest ? "/" : "/to-receive") as any);
                  }}
                >
                  <Text style={styles.trackOrderText}>
                    {isGuest ? "Về trang chủ" : "Theo dõi đơn hàng"}
                  </Text>
                </TouchableOpacity>
              </>
            )}
            ...
            <View style={webStyles.sheetHeader}>
              <View>
                ...
                <TouchableOpacity onPress={openAddAddress}>
                  <Text style={webStyles.addAddressLink}>
                    + Thêm địa chỉ mới
                  </Text>
                </TouchableOpacity>
              </View>
                ...
                allAddresses.map((addr) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        webStyles.addressItem,
                        isSelected && webStyles.addressItemSelected,
                      ]}
                      activeOpacity={0.7}
                    >
                      <View style={{ flex: 1, marginRight: 12 }}>
                        <View style={webStyles.addressNameRow}>
                          <Text style={webStyles.addressName}>
                            {addr.receiver_name}
                          </Text>
                          <Text style={webStyles.addressPhone}>
                            | {addr.phone_number}
                          </Text>
                          {addr.is_default && (
                            <View style={webStyles.defaultBadge}>
                              <Text style={webStyles.defaultBadgeText}>
                                Mặc định
                              </Text>
                            </View>
                          )}
                        ...
                        <Text style={webStyles.addressDetail}>
                          {addr.street_address},{" "}
                          {addr.ward_commune ? `${addr.ward_commune}, ` : ""}
                          {addr.district}, {addr.province_city}
                        </Text>
                      </View>
                      <View style={webStyles.addressActions}>
                        ...
                        <View
                          style={[
                            webStyles.radio,
                            isSelected && webStyles.radioSelected,
                          ]}
                        >
                  ...
                  <View key={voucher.id} style={styles.voucherCard}>
                    ...
                    <View style={styles.voucherContent}>
                      ...
                      {!isEligible && missingAmount > 0 && (
                        ...
                        >
                          * Mua thêm {missingAmount.toLocaleString("vi-VN")}đ để
                          sử dụng mã này
                        </Text>
                      )}
                      <TouchableOpacity
